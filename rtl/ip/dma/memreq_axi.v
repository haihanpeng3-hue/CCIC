...
assign urat_debug_wreq = mem_req & mem_we;
assign urat_debug_rreq = mem_req & (~mem_we);
...
always @(posedge clk or negedge rst_n) begin
        if(~rst_n)begin
            ...
            if(urat_debug_wreq)begin
                mem_addr_r <= mem_addr & 32'hffff_fffc;
                if(mem_stb) begin
                    case (mem_addr[1:0])
                        2'h0 : begin
                            mem_wstrb <= 4'b0001;
                        ...
                        2'h1 : begin
                            mem_wstrb <= 4'b0010;
                            mem_wdata_r <= {16'h0,mem_wdata[7:0],8'h0};
                        end
                        2'h2 : begin
                            mem_wstrb <= 4'b0100;
                            mem_wdata_r <= {8'h0,mem_wdata[7:0],16'h0};
                        end
                        2'h3 : begin
                            mem_wstrb <= 4'b1000;
                            mem_wdata_r <= {mem_wdata[7:0],24'h0};
                        end
                        default : begin
                            mem_wstrb <= 4'b0000;
                            mem_wdata_r <= 32'h0;
