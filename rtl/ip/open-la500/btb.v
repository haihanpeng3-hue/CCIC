...
(
    ...
    output            taken         ,
    output            ret_en        ,
    output [ 5:0]     ret_index     ,
    //from id
    input             operate_en    ,
    input  [31:0]     operate_pc    ,
    input  [ 5:0]     operate_index ,
    input             pop_ras       ,
    input             push_ras      ,
    input             add_entry     ,
    input             delete_entry  ,
...
localparam integer RAS_PTR_W = $clog2(RASNUM + 1);

reg [29:0] ras [RASNUM-1:0];
...
wire [ 5:0] btb_match_index;
wire [ 5:0] btb_random_index;

wire [ 3:0] ras_match_index;
wire [ 3:0] ras_random_index;

wire btb_all_entry_valid;
wire [5:0] btb_select_one_invalid_entry;
wire [5:0] btb_add_entry_index;
reg  [5:0] btb_add_entry_index_r;
wire [63:0] btb_add_entry_dec;

wire ras_all_entry_valid;
wire [3:0] ras_select_one_invalid_entry;
wire [3:0] ras_add_entry_index;

wire [63:0] btb_weak_entry;
reg  [63:0] btb_weak_entry_r;
wire [63:0] btb_weak_entry_t;
reg         btb_add_entry_r;
wire [5:0]  btb_sel_one_weak_entry;
wire        btb_has_one_weak_entry;
...
always @(posedge clk) begin
	btb_weak_entry_r      <= btb_weak_entry;
	btb_add_entry_r       <= operate_en && !pop_ras && add_entry;
	btb_add_entry_index_r <= btb_add_entry_index;
end

// prefer evicting weakly not-taken entries first, while avoiding the just-allocated slot
assign btb_weak_entry_t = btb_weak_entry_r & ({64{!btb_add_entry_r}} | ~btb_add_entry_dec); 
assign btb_has_one_weak_entry = |btb_weak_entry_t;

decoder_6_64 dec_btb_add_entry (.in(btb_add_entry_index_r), .out(btb_add_entry_dec));
one_valid_64 sel_one_weak_entry (.in(btb_weak_entry_t), .out_en(btb_sel_one_weak_entry));

//assign btb_random_index = (btb_match && (fcsr == btb_match_index)) ? (fcsr+1'b1) : fcsr;
// weak not-taken entries can exit first and help preserve strongly taken hot branches. 
assign btb_add_entry_index = !btb_all_entry_valid ?      btb_select_one_invalid_entry :
							 btb_has_one_weak_entry ? btb_sel_one_weak_entry : 
							 			 fcsr ; 

assign btb_all_entry_valid = &btb_valid;
one_valid_64 sel_one_btb_entry (.in(~btb_valid), .out_en(btb_select_one_invalid_entry));
...
always @(posedge clk) begin
    if (reset) begin
        ...
        if (add_entry) begin
            ...
            btb_counter[btb_add_entry_index] <= 2'b11;
        end
        else if (delete_entry) begin
            btb_valid[operate_index]         <= 1'b0;
            btb_counter[operate_index]       <= 2'b00;
        end
        else if (target_error) begin
            btb_target[operate_index]    <= right_target[31:2];
            btb_counter[operate_index]   <= 2'b11;
        end
...
generate 
	for (i = 0; i < BTBNUM; i = i + 1)
		begin: sel_weak_entry
			assign btb_weak_entry[i] = btb_valid[i] && !btb_counter[i][1];
		end
endgenerate
...
assign ras_top = ras[ras_ptr - 1'b1]; //ras modify may before inst fetch

encoder_64_6 encode_btb_match (.in(btb_match_rd), .out(btb_match_index));
...
assign ret_index = {6{btb_match}} & {btb_match_index} | 
				   {6{ras_match}} & {2'b0,ras_match_index};

assign ras_full  = (ras_ptr == RASNUM[RAS_PTR_W-1:0]);
assign ras_empty = (ras_ptr == {RAS_PTR_W{1'b0}});

always @(posedge clk) begin
    if (reset) begin
			ras_ptr <= {RAS_PTR_W{1'b0}};
    end
