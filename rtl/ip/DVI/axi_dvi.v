...
(
    ...
    reg [31:0] dvi_rect_dir_active,dvi_rect_l_w_active,dvi_squ_dir_active,dvi_squ_r_active;
...
//-------------------------------{dvi controller}begin----------------------------//
    ...
    wire start_of_frame = (hdata == {WIDTH{1'b0}}) && (vdata == {WIDTH{1'b0}});

    always @(posedge aclk) begin
        if(!aresetn) begin
            dvi_rect_dir_active <= 32'h0;
            dvi_rect_l_w_active <= 32'h0;
            dvi_squ_dir_active <= 32'h0;
            dvi_squ_r_active <= 32'h0;
        end
        else if (start_of_frame) begin
            dvi_rect_dir_active <= DVI_RECT_DIR;
            dvi_rect_l_w_active <= DVI_RECT_L_W;
            dvi_squ_dir_active <= DVI_SQU_DIR;
            dvi_squ_r_active <= DVI_SQU_R;
        end
    end
    ...
    always @(posedge aclk) begin
        if (!aresetn)
            hdata <= {WIDTH{1'b0}};
        else if (hdata == (HMAX - 1))      // If horizontal counter reaches max
    ...
    assign hdata_in_range = (hdata > (dvi_rect_dir_active[31:16]-dvi_rect_l_w_active[31:16])) && (hdata < (dvi_rect_dir_active[31:16]+dvi_rect_l_w_active[31:16]));
    // Check if vdata is in the range specified by DVI_RECT_L_W
    assign vdata_in_range = (vdata > dvi_rect_dir_active[15:0]) && (vdata <(dvi_rect_dir_active[15:0]+dvi_rect_l_w_active[15:0]));
    
    assign hdata1_in_range = (hdata > (dvi_squ_dir_active[31:16]-dvi_squ_r_active[31:16])) && (hdata < (dvi_squ_dir_active[31:16]+dvi_squ_r_active[31:16]));
    // Check if vdata is in the range specified by DVI_RECT_L_W
    assign vdata1_in_range = (vdata > (dvi_squ_dir_active[15:0]-dvi_squ_r_active[15:0])) && (vdata <(dvi_squ_dir_active[15:0]+dvi_squ_r_active[15:0]));
